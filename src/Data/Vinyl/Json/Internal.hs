...
class BuildHList rs res where
  buildHList' :: V.HList rs -> res

instance RReverse rs sr => BuildHList rs (V.HList sr) where
  buildHList' rs = rReverse rs

instance (BuildHList (r ': rs) res)
         => BuildHList rs (r -> res) where
  buildHList' rs = \a -> buildHList' (V.Identity a :& rs)
...
mkHList :: forall rs. (BuildHList '[] (V.Curried rs (V.HList rs)))
        => V.Curried rs (V.HList rs)
mkHList = buildHList' V.RNil
